...
if (empty($_GET['offer'])) {
    header('Location: list-cards.php');
    exit();
}

$offer = Offer::load($_GET['offer']);
if (!$offer) {
    header('Location: list-cards.php');
    exit();
}
...
if (!empty($_POST)) {
    ...
    $emailAnnouncer = $offer->getContact()->getEmail();
...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <?= $offer->getCategory()->getName() . ' | ' . $offer->getCity()->getName() ?>
        </div>
        <div class="card-body">
    ...
    <?php if ($offer->getContact()->isVisible()): ?>
        <div class="row">
            ...
            <ul style="margin-left: 1rem;">
                <li><?= $offer->getContact()->getFirstName() . ' ' . $offer->getContact()->getName() ?></li>
                <li>Téléphone : <a href="tel:<?= $offer->getContact()->getPhone() ?>"><?= $offer->getContact()->getPhone() ?></a></li>
                <li>Email : <a href="mailto:<?= $offer->getContact()->getEmail() ?>"><?= $offer->getContact()->getEmail() ?></a></li>
            </ul>
            <br>
        </div>
    <?php else: ?>
        <div class="row justify-content-center">
            <form method="post" id="contact-announcer" name="contact-announcer" action="contact-announcer.php?offer=<?= $offer->getId() ?>">
