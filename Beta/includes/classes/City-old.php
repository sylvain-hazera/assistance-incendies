...
class City-test
{
    ...
    public static function load($id) {
        ...
        $sql = $db->prepare("SELECT * FROM city-test WHERE id = :id");
        $sql->bindValue(":id", $id);
    ...
    public static function loadAll() {
        ...
        $sql = $db->prepare("SELECT * FROM city-test ORDER BY name ASC");
        if($sql->execute()){
            while($row = $sql->fetch()){
    ...
    public static function loadCitiesWithOffer() {
        ...
        $sql = $db->prepare("SELECT DISTINCT city.* FROM city-test, offer WHERE city-test.id = offer.city_id ORDER BY city.name ASC");
        if($sql->execute()){
            while($row = $sql->fetch()){
                $cities[] = self::objectFromResult($row);
            }
