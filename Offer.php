...
class Offer
{
    ...
    public function save(){
        ...
        try {
            ...
            $db->beginTransaction();
            $sql = $db->prepare("INSERT INTO offertest values(
                         0,
                         :city_id,
                         :category_id,
                         :description,
    ...
    public static function load($id) {
        ...
        $sql = $db->prepare("SELECT * FROM offertest WHERE id = :id");
        $sql->bindValue(":id", $id);
    ...
    public static function loadWithFilters($filters){
        ...
        $request_select = "SELECT * FROM offertest WHERE type_id = 1";
        $request_count = "SELECT COUNT(*) AS count FROM offertest WHERE type_id = 1";
    ...
    public static function loadByToken($token) {
        ...
        $sql = $db->prepare("SELECT * FROM offertest WHERE token = :token LIMIT 1");
    ...
    public static function loadAll() {
        ...
        $sql = $db->prepare("SELECT * FROM offertest ORDER BY date_creation DESC");
        if($sql->execute()){
            while($row = $sql->fetch()){
                $offers[] = self::objectFromResult($row);
            }
    ...
    public static function totalOffers(){
        ...
        $sql = $db->prepare("SELECT COUNT(*) AS count FROM offertest");
        if($sql->execute()){
            $row = $sql->fetch();
            return $row['count'];
        }
    ...
    public static function deleteByToken($token){
        ...
        $sql = $db->prepare("DELETE FROM offertest WHERE token = :token");
        $sql->bindValue(":token", $token);
        $sql->execute();
        $count = $sql->rowCount();
