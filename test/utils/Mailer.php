...
function sendMailContactAnnouncer($emailAnnouncer, $type, $name, $firstname, $phone = '', $email, $move, $category, $city, $description, $token)
{
    ...
    $message .= "<table><tr><td><img src='http://assist-crise.fr/img/logo.png'></td><td><a href='" . SITE_URL . "'>Assista Crise</a></td></tr></table>";

    sendMail($emailAnnouncer, "Urgences - mise en relation - Demande", $message);
}
function sendMailCreate($Create)
{
    $message = "<h4>Urgences - Mise en relation</h4>";
    $message = "<p>Nouvelle création de comptes</p><br><br>";

    $message .= "<ul>";
    $message .= "Veuillez consulter la BDD pour obtenir l'adresse mail.";

    sendMail(contact@assista-crise.fr, "assista-crise - Creation de compte", $message);
}
...
function sendMailOffer($target, $token, $category, $city, $description)
{
    ...
    $message .= "<table><tr><td><img src='http://assista-crise.fr/img/logo.png'></td><td><a href='" . SITE_URL . "'>Assista Crise</a></td></tr></table>";
